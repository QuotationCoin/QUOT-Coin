...
#include "zquotationcontroldialog.h"
#include "spork.h"
                                                          ...
                                                          currentBalance(-1)
{
    ...
    // "Spending 999999 zQUOT ought to be enough for anybody." - Bill Gates, 2017
    ui->zQUOTpayAmount->setValidator( new QDoubleValidator(0.0, 21000000.0, 20, this) );
    ...
    ui->labelzQUOTSyncStatus->setText("(" + tr("out of sync") + ")");
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        ui->pushButtonMintzQUOT->setEnabled(false);
        ui->pushButtonMintzQUOT->setToolTip(tr("zQUOT is currently disabled due to maintenance."));

        ui->pushButtonSpendzQUOT->setEnabled(false);
        ui->pushButtonSpendzQUOT->setToolTip(tr("zQUOT is currently disabled due to maintenance."));
    }
}
...
void PrivacyDialog::on_addressBookButton_clicked()
{
    ...
    if (dlg.exec()) {
        ...
        ui->zQUOTpayAmount->setFocus();
    }
}

void PrivacyDialog::on_pushButtonMintzQUOT_clicked()
{
    if (!walletModel || !walletModel->getOptionsModel())
        return;
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zQUOT is currently undergoing maintenance."), QMessageBox::Ok,
                                 QMessageBox::Ok);
        return;
    }
    ...
    ui->TEMintStatus->setPlainText(tr("Minting ") + ui->labelMintAmountValue->text() + " zQUOT...");
    ...
    QString strStatsHeader = tr("Successfully minted ") + ui->labelMintAmountValue->text() + tr(" zQUOT in ") +
...
void PrivacyDialog::on_pushButtonSpendzQUOT_clicked()
{
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zQUOT is currently undergoing maintenance."), QMessageBox::Ok, QMessageBox::Ok);
        return;
    }
    ...
    if (encStatus == walletModel->Locked || encStatus == walletModel->UnlockedForAnonymizationOnly) {
        ...
        // Wallet is unlocked now, sedn zQUOT
        sendzQUOT();
        return;
    }
    // Wallet already unlocked or not encrypted at all, send zQUOT
    sendzQUOT();
}

void PrivacyDialog::on_pushButtonZQuotationControl_clicked()
{
    ZQuotationControlDialog* zQUOTControl = new ZQuotationControlDialog(this);
    zQUOTControl->setModel(walletModel);
    zQUOTControl->exec();
}

void PrivacyDialog::setZQuotationControlLabels(int64_t nAmount, int nQuantity)
{
    ui->labelzQUOTSelected_int->setText(QString::number(nAmount));
...
void PrivacyDialog::sendzQUOT()
{
    ...
    if(ui->payTo->text().isEmpty()){
        ...
        if (!address.IsValid()) {
            QMessageBox::warning(this, tr("Spend Zerocoin"), tr("Invalid Quotation Address"), QMessageBox::Ok, QMessageBox::Ok);
            ui->payTo->setFocus();
            return;
        }
    }
    ...
    double dAmount = ui->zQUOTpayAmount->text().toDouble();
    ...
    // Warn for additional fees if amount is not an integer and change as zQUOT is requested
    ...
    if(!fWholeNumber && fMintChange){
        ...
        strFeeWarning += QString::number(dzFee, 'f', 8) + " QUOTATION </b>will be added to the standard transaction fees!<br />";
    ...
    QString strAmount = "<b>" + QString::number(dAmount, 'f', 8) + " zQUOT</b>";
    ...
    // use mints from zQUOT selector if applicable
    vector<CZerocoinMint> vMintsSelected;
    if (!ZQuotationControlDialog::listSelectedMints.empty()) {
        vMintsSelected = ZQuotationControlDialog::GetSelectedMints();
    }
    ...
    // Clear zquotation selector in case it was used
    ZQuotationControlDialog::listSelectedMints.clear();
    ...
    for (CZerocoinSpend spend : receipt.GetSpends()) {
        strStats += tr("zQUOT Spend #: ") + QString::number(nCount) + ", ";
    ...
    for (const CTxOut& txout: wtxNew.vout) {
        strStats += tr("value out: ") + FormatMoney(txout.nValue).c_str() + " Quotation, ";
        nValueOut += txout.nValue;

        strStats += tr("address: ");
        CTxDestination dest;
        if(txout.scriptPubKey.IsZerocoinMint())
            strStats += tr("zQUOT Mint");
                               ...
                               const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    for (const auto& denom : libzerocoin::zerocoinDenomList) {
        ...
        strDenomStats = strUnconfirmed + QString::number(mapDenomBalances.at(denom)) + " x " +
                        ...
                        QString::number(nSumPerCoin) + " zQUOT </b>";
    ...
    ui->labelzAvailableAmount->setText(QString::number(zerocoinBalance/COIN) + QString(" zQUOT "));
    ui->labelzAvailableAmount_2->setText(QString::number(matureZerocoinBalance/COIN) + QString(" zQUOT "));
    ui->labelzQUOTAmountValue->setText(BitcoinUnits::floorHtmlWithUnit(nDisplayUnit, balance - immatureBalance - nLockedBalance, false, BitcoinUnits::separatorAlways));
}
...
{
    ui->labelzQUOTSyncStatus->setVisible(fShow);
}
