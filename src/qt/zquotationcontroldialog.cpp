...
#include "ui_zquotationcontroldialog.h"
...
std::list<std::string> ZQuotationControlDialog::listSelectedMints;
std::list<CZerocoinMint> ZQuotationControlDialog::listMints;

ZQuotationControlDialog::ZQuotationControlDialog(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::ZQuotationControlDialog),
    model(0)
{
    ui->setupUi(this);
    listMints.clear();
...
ZQuotationControlDialog::~ZQuotationControlDialog()
{
    delete ui;
}

void ZQuotationControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZQuotationControlDialog::updateList()
{
...
void ZQuotationControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZQuotationControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CZerocoinMint mint : listMints) {
    ...
    ui->labelZQuotation_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(listSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZQuotationControlLabels(nAmount, listSelectedMints.size());
}

std::vector<CZerocoinMint> ZQuotationControlDialog::GetSelectedMints()
{
...
void ZQuotationControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
